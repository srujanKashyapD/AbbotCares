...
import {
  PhoneGroup,
  PhoneNumberService,
} from '../../core/services/phone-number.service';
...
export class MultiformSignupComponent implements OnInit, OnDestroy {
  ...
  get address() {
    return this.registrationDetails.controls;
  }
  ...
  submit(): void {
    if (this.step === 3) {
      ...
      this.apiService
        .validateOtp(phone, this.otpDetails)
        .pipe(
          filter((data: Validate) => data.status.success === true),
          switchMap((val) =>
            this.apiService.getCustomerByPhone(phone).pipe(
              tap((data) => {
                if (data.status.code !== 500) this.router.navigate(['login']);
              })
            )
          ),
          switchMap((val) =>
            this.apiService
              .getCustomerByEmail(this.registrationDetails.value.email)
              .pipe(
                tap((data) => {
                  if (data.status.code !== 500) this.router.navigate(['login']);
                })
  ...
  private validateEmail(mobile: string, email: string): Observable<boolean> {
    ...
    return this.apiService.validateEmail(mobile, email).pipe(
      switchMap((response) => {
        if (response.status === 200) {
          return this.apiService
            .generateSession(customerObj)
            .pipe(
              concatMap((data: Validate) =>
