...
export class ApiServiceService {
  ...
  addNewCustomer(name:{firstName: string, lastName: string}, email: string, chosenPlan: string)
  : Observable<Validate> {
    const url = this.API_URL + 'mobile/v2/api/customer/add';
    const now = new Date();
    const registeredDate = `${now.getFullYear()}-${now.getMonth() + 1}-${now.getDate()} ${now.getHours()}:${now.getMinutes}:${now.getSeconds}`
    const payload = {
      "root": {
        "customer":[
          {
    ...
    return this.http.post<Validate>(url, payload, { headers: this.getHeadersWithAuthToken()});
  }
  ...
  validateEmail(mobile: string, email: string) {
    ...
    return this.http.get(url, {
      headers: new HttpHeaders({ 'country-code': 'MY', 'language': 'en_US', 'Authorization': '85a4447a3b438fbb6e33e5c0428460d3' }) ,
      observe: 'response'
    });
  }
  ...
  validateOtp(mobile: string, otp: string): Observable<Validate> {
    ...
    .pipe(tap((data: Validate) => localStorage.setItem('token', data?.auth.token ? data.auth.token : '')));
  }
  ...
  getCustomerData(mlp: boolean, gapToUpgradeFor: {enabled: boolean, gapDays: number}, slabHistory: boolean): Observable<CustomerDetail>  {
    ...
    return this.http.get<CustomerDetail>(getCustomerUrl, { headers: this.getHeadersWithAuthToken() });
  }

  getCustomerByEmail(email: string): Observable<Validate> {
    const url = this.API_URL + `mobile/v2/api/customer/getbyemail?email=${email}`;
    return this.http.get<Validate>(url, { headers: this.getHeadersWithAuthToken() });
  }
